...
def main(args) -> None:
    ...
    for pdb in input_dict:
        ...
        S_new = feature_dict["S"].clone()
        for sequence in input_dict[pdb]:
            S_current = feature_dict["S"].clone()
            current_chain = ''
            chain_pos = 0
            for i, c in enumerate(protein_dict["chain_letters"].tolist()):
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        ...
        default="",
        required=True,
        help="Path to json listing PDB paths associated to the sequences you want to score. {'/path/to/pdb': [{chain_id: sequence, ...}, ...]}",
    )
